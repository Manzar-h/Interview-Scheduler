...
describe("Application", () => {
    ...
    it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
      ...
      //&
      // 4. Check that the input field for student name is pre-filled with the current student name.
      const appointments = getAllByTestId(container, "appointment");
      const appointment = appointments.find(
        appointment => queryByText(appointment, "Archie Cohen")
      );  
      fireEvent.click(queryByAltText(appointment, "Edit"));
      // 5. Change the student name to "Jane Smith"
      ...
      expect(getByText(appointment, "Saving")).toBeInTheDocument();
       // 8. Wait until the element with the text "Jane Smith" is displayed.
      await waitForElement(() => getByText(appointment, "Jane Smith"));
  
      expect(getByText(appointment, "Jane Smith")).toBeInTheDocument();
